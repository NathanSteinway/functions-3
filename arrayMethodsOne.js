...
const totalPopulation = populations.reduce((runningTotal, curElement, curIndex, wholeArray) => {
  return runningTotal + curElement + curIndex
})
...
const bestMap = orders.map((element) => {
...
const impulseBuy = purchases.filter((element) => element.owner === "Bob").reduce((acc, curr) => acc + curr.price, 0)
